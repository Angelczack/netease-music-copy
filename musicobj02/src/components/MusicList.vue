        ...
        <div class="mlist">
            <div class="swiper-container" id="musicSwiper">
                <div class="swiper-wrapper">
                    <router-link :to="{path:'/listdetail',query:{id:item.id}}" class="swiper-slide" v-for="item in musicls.musics" :key="item.id">
                        <img :src="item.picUrl" />
                        <div class="name">{{ item.name }}</div>
                        <div class="count">
                            <svg class="icon" aria-hidden="true">
                                <use xlink:href="#icon-fanjutuijain"></use>
                            </svg>
...
import { getMlists } from "@/api/index.js"   //@指的是项目目录 src
export default {
    name: "musiclist",
    setup() {
        function changeValue(num) {  //格式化播放数
            var res = 0;
            if (num >= 100000000) {
                res = num / 100000000;
